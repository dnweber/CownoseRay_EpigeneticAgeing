...
#Bayesian model
bayes_model <- stan_glmer(cbind(Meth, Total - Meth) ~ Age.Days * Tissue * Sex + (1 | Fish), data = tmp3, family = binomial(link = "logit"), cores = 4, iter=50000)
...
test1 <- data.frame(emmeans(bayes_model, ~ Age.Days * Tissue * Sex, type = "response"))
...
#Jason test
jtest1 <- variable_importance(bayes_model, 0.95, c('Age.Days','Tissue','Sex'))
